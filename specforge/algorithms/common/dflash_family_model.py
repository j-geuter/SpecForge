...
class OnlineDFlashModel(nn.Module):
    ...
    def __init__(
        ...
        kl_decay: float = 1.0,
        metric_top_k: int = 16,
        normalize_by_anchors: bool = False,
    ):
        super().__init__()
        if metric_top_k <= 0:
        ...
        self._normalize_by_anchors = bool(normalize_by_anchors)
        ...
        if candidate_selector is not None and (
    ...
    @property
    def normalize_by_anchors(self) -> bool:
        return self._normalize_by_anchors or self.loss_type in _DPACE_LOSS_TYPES
    ...
    def _dflash_objective_chunk_terms(
        ...
        target_pred_indices: Optional[torch.Tensor] = None,
        *,
        ...
        if self.loss_type in _OVERLAP_LOSS_TYPES:
            ...
            with torch.no_grad():
                if target_logits is None:
                    ...
                    if target_pred_indices is None:
                        raise ValueError(
                            "cached teacher logits require prediction indices"
                        )
                    indices = target_pred_indices.reshape(batch_size, -1, 1).expand(
                        -1, -1, target_logits.shape[-1]
                    )
                    aligned_logits = target_logits.gather(1, indices).reshape_as(
                        objective_logits
                    )
                log_p = F.log_softmax(aligned_logits.float(), dim=-1)
            ...
            with torch.no_grad():
                credit_probability = (
                    torch.exp(torch.minimum(log_p, log_q)).sum(-1).clamp(0.0, 1.0)
                )
        ...
        if self.normalize_by_anchors:
            if sequence_anchor_scale is None:
                raise ValueError("precomputed full-sequence anchor scale is required")
    ...
    def _dflash_metric_chunk_terms(
        ...
        *,
        ...
        credit_probability = unary.hard_label_probability
        if self.loss_type in _OVERLAP_LOSS_TYPES:
            ...
            with torch.no_grad():
                ...
                teacher_probabilities = (
                    self.lm_head(aligned_target_hidden.reshape(b, a * d, h))
                    .float()
                    .softmax(-1)
                    .reshape_as(unary.probabilities)
                )
                credit_probability = (
                    torch.minimum(unary.probabilities, teacher_probabilities)
                    .sum(-1)
                    .clamp(0.0, 1.0)
                )
    ...
    def _dflash_metric_loss_weights(
        self,
        credit_probability: torch.Tensor,
        weight_mask: torch.Tensor,
        sequence_anchor_scale: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        if self.loss_type == "dflash":
            scale = sequence_anchor_scale if self.normalize_by_anchors else 1.0
            if scale is None:
                raise ValueError("precomputed full-sequence anchor scale is required")
            if self.loss_decay_gamma is None or self.loss_decay_gamma <= 0:
                return weight_mask * scale
            positions = torch.arange(
                weight_mask.shape[-1],
            ...
            return weight_mask * decay_weights * scale
    ...
    def _dflash_teacher_terms(
        ...
        *,
        ...
        teacher_ids = teacher_probabilities.argmax(dim=-1)
        expected_acceptance = (
    ...
    def forward(
        ...
        objective = self._dflash_objective_chunk_terms
        target_pred_indices = None
        if self.loss_type in _OVERLAP_LOSS_TYPES:
            # Project each teacher position once, outside activation recomputation.
            # Gather only the current anchor chunk to avoid a full anchor-vocab cache.
            with torch.no_grad():
                target_logits = self.lm_head(target_last_hidden_states)
            target_pred_indices = (safe_label_indices - 1).clamp(min=0)
            objective = partial(objective, target_logits=target_logits)
        ...
        ratio_metrics = {
            ...
            (
                f"{self.loss_type}_loss"
                if self.loss_type in _OVERLAP_LOSS_TYPES
                else "lk_loss" if self.lk_loss_type is not None else "ce_loss"
            ): (
